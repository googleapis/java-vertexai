 ...
 *
 * <p>mvn exec:java
 * -Dexec.mainClass="com.google.cloud.agentplatform.examples.AgentEngineSessionEvents"
 */
...
import com.google.cloud.agentplatform.types.AppendAgentEngineSessionEventResponse;
...
import com.google.cloud.agentplatform.types.ListAgentEngineSessionEventsResponse;
import com.google.common.collect.ImmutableMap;
import java.time.Instant;

/** An example of using the Java SDK to perform operations on Agent Engine session events. */
public final class AgentEngineSessionEvents {
  public static void main(String[] args) {
    ...
    try {
      ...
      System.out.println("\n--- Appending session event ---");
      AppendAgentEngineSessionEventResponse appendResponse =
          client.agentEngines.sessions.events.append(
              sessionName, "user_123", "invocation_abc", Instant.now(), null);
      System.out.println("Appended session event.");

      // 3. List session events
      System.out.println("\n--- Listing session events ---");
      ListAgentEngineSessionEventsResponse listResponse =
          client.agentEngines.sessions.events.privateList(sessionName, null);
      System.out.println(
          "Session events found: "
              + listResponse.sessionEvents().map(java.util.List::size).orElse(0));

      // 4. Delete session
      System.out.println("\n--- Deleting session ---");
      DeleteAgentEngineSessionOperation deleteOp =
          client.agentEngines.sessions.delete(sessionName, null);
      System.out.println("Delete session operation started.");
  ...
  private AgentEngineSessionEvents() {}
}
